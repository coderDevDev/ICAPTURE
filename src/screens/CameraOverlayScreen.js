...
export default function CameraOverlayScreen({ navigation, route }) {
  const { template, templateInfo, answerKey, selectedStudent } = route.params;
...
<body>
    ...
    function extractDocument() {
      ...
      img.onload = function() {
        try {
          ...
          // Extract at optimized resolution: 1920x1280 (landscape, balanced quality/size)
          const resultCanvas = scanner.extractPaper(img, 1920, 1280);
          ...
          // Optimized JPEG quality for ~500KB file size
          const extractedBase64 = canvas.toDataURL('image/jpeg', 0.85).split(',')[1];
  ...
  const handleProceedToResults = async () => {
    if (!extractedImageUri) {
      Alert.alert('Error', 'No extracted image available to process');
      return;
    }

    try {
      // Process the image the SAME WAY as the download function
      const timestamp = Date.now();
      const processedUri = `${FileSystem.cacheDirectory}omr_processed_${timestamp}.jpg`;
      
      // Read the base64 and re-save with explicit JPEG format
      if (extractedImageBase64) {
        await FileSystem.writeAsStringAsync(processedUri, extractedImageBase64, {
          encoding: FileSystem.EncodingType.Base64
        });
      } else {
        await FileSystem.copyAsync({
          from: extractedImageUri,
          to: processedUri
        });
      }

      // Use manipulateAsync to ensure proper JPEG format
      ...
      // Navigate to RectanglePreview which will call backend API
      // This is the SAME flow as upload image - ensures consistent results
      navigation.navigate('RectanglePreview', {
        imageUri: finalImage.uri,
        originalUri: finalImage.uri,
        template,
        templateInfo,
        assetId: null,
        ...
        answerKey,
        selectedStudent,
        skipToResults: true // Auto-proceed after backend processing
      });
    } catch (error) {
  ...
  const capturePhoto = async () => {
    ...
    try {
      const photo = await cameraRef.current.takePictureAsync({
        quality: 1.0, // Maximum quality for better extraction
        base64: false,
      ...
      // Resize to higher resolution for better OMR processing
      const optimizedImage = await manipulateAsync(
        photo.uri,
        [
          {
            resize: {
              width: 2400, // Increased from 1920 for better quality
              height: undefined
            }
          }
        ],
        {
          compress: 0.9, // Higher compression quality (was 0.85)
          format: SaveFormat.JPEG
        }
      );
  ...
  const handleDownloadExtracted = async () => {
    if (!extractedImageUri) {
      Alert.alert('Error', 'No extracted image available to download');
      return;
    }
    ...
    try {
      ...
      // Re-process the image to ensure high quality JPEG format
      const timestamp = Date.now();
      const highQualityUri = `${FileSystem.cacheDirectory}omr_extracted_${timestamp}.jpg`;
      
      // Read the base64 and re-save with explicit JPEG format
      if (extractedImageBase64) {
        await FileSystem.writeAsStringAsync(highQualityUri, extractedImageBase64, {
          encoding: FileSystem.EncodingType.Base64
        });
      } else {
        // If no base64, copy the existing file
        await FileSystem.copyAsync({
          from: extractedImageUri,
          to: highQualityUri
        });
      ...
      // Use manipulateAsync to ensure proper JPEG format with optimized size
      const finalImage = await manipulateAsync(
        highQualityUri,
        [], // No transformations, just format conversion
        {
          compress: 0.85, // Optimized quality for ~500KB file size
          format: SaveFormat.JPEG
        }
      );

      // Save to media library with proper JPEG format
      const asset = await MediaLibrary.createAssetAsync(finalImage.uri);
      
      // Create album if it doesn't exist
      const album = await MediaLibrary.getAlbumAsync('OMR Scanner');
      if (album == null) {
        await MediaLibrary.createAlbumAsync('OMR Scanner', asset, false);
      } else {
      ...
      try {
        await FileSystem.deleteAsync(highQualityUri, { idempotent: true });
      } catch (cleanupError) {
      ...
      Alert.alert(
        '✅ Downloaded!',
        'Optimized JPEG (~500KB) saved to gallery in "OMR Scanner" album',
        [{ text: 'OK' }]
      );
    } catch (error) {
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalButtonsContainer}>
              <TouchableOpacity
                ...
                onPress={handleDownloadExtracted}
                disabled={!extractedImageUri || downloading}>
            ...
            <TouchableOpacity
              style={[styles.modalProceedButton, !extractedImageUri && styles.modalButtonDisabled]}
