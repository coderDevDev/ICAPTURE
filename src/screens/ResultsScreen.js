...
export default function ResultsScreen({ navigation, route }) {
  const { imageUri, template, templateInfo, assetId, answerKey, selectedStudent } = route.params;
    ...
    try {
      ...
      if (answerKey) {
        ...
        try {
          await saveResult({
            id: `result_${Date.now()}`,
            studentId: selectedStudent?.id || 'anonymous',
          ...
          console.log('✅ Result saved successfully for student:', selectedStudent?.name || 'Anonymous');
        } catch (saveError) {
