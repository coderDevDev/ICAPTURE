...
export default function DocumentExtractionScreen({ navigation, route }) {
  const { imageUri, template, templateInfo, answerKey, selectedStudent } = route.params;
      ...
      try {
        ...
        img.onload = function() {
          try {
            ...
            // Using optimized resolution for balanced quality/size (landscape)
            ...
            // Convert canvas to base64 with optimized quality (~500KB)
            const extractedBase64 = canvas.toDataURL('image/jpeg', 0.85);
