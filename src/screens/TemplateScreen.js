...
import { getAllAnswerKeys, getSettings, getAllStudents } from '../services/database';
...
export default function TemplateScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    // Pre-select student if passed from StudentsScreen
    if (route.params?.selectedStudent) {
      setSelectedStudent(route.params.selectedStudent);
    }
  }, []);
    ...
    try {
      ...
      if (isUpload && uploadedImageUri) {
        ...
        navigation.navigate('Camera', {
          ...
          selectedStudent: selectedStudent // Can be null if no student selected
        });
      }
    } catch (error) {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {selectedTemplate && (
          <Card style={styles.studentCard}>
            <Card.Content>
              <Title style={styles.sectionTitle}>👨‍🎓 Student Selection</Title>
              <Paragraph style={styles.sectionText}>
                Select a student for this exam (optional but recommended for tracking)
              </Paragraph>

              {students.length === 0 ? (
                <View style={styles.noStudents}>
                  <Paragraph style={styles.noStudentsText}>
                    ℹ️ No students available. Add students to track their performance.
                  </Paragraph>
                  <Button
                    mode="outlined"
                ...
                <RadioButton.Group
                  ...
                  <List.Item
                    title="No Student (Anonymous)"
                    description="Scan without linking to a student"
                    left={() => <RadioButton.Android value="" />}
                    style={!selectedStudent && styles.selectedListItem}
                  />
                  {students.map(student => (
                    <List.Item
                      ...
                      left={() => <RadioButton.Android value={student.id} />}
                      style={
